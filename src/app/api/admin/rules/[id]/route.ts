...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (patch.summary || patch.legalRule) assertPermission(session.user.role, "rules:write");
    if (patch.publicationStatus || patch.reviewStatus) assertPermission(session.user.role, "review:write");
