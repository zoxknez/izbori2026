...
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.batch([
      ...
      db.insert(datasetVersions).values({ id: datasetId, version, status: "active", updatePriority: "normal", manifestHash: sha256, publishedAt: new Date(), publishedBy: session.user.id }),
      ...
      db.insert(auditLog).values({ id: crypto.randomUUID(), actorUserId: session.user.id, action: "publish", entityType: "dataset_version", entityId: datasetId, after: { version, sha256 } }),
    ]);
